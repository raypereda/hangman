...
public class HangmanGame {
  /**
   * An enumeration for the current state of the game
   */
  ...
  private static boolean debuggingIsOn = false;

  
  /**
   * sets a switch used for toggling debug print statements
   * This is less cumbersome than logging for a small program
   * @param status of the switch for debugging
   */
  public static void setDebugging(boolean status) {
	  debuggingIsOn = status;
  }
  ...
  /**
   * @return The score for the current game state <br>
   * There are 26 letters. If you needed to pick every letter in order
   * to win, then you really didn't win. 25 is set to the worst possible
   * score. A low score represents a good set of guesses so far.
   */
  ...
  public String toString() {
    return getGuessedSoFar() + 
    		"; score=" + currentScore() + 
    		"; wrongGuessesRemaining="  + numWrongGuessesRemaining() +
    		"; status=" + gameStatus();
  }
  
  /**
   * @param guessingStrategy the same strategy is used through the whole game
   * @return the score for the game until it is won or lost
   */
  public int score2(GuessingStrategy guessingStrategy) {
	  ...
	  while (this.gameStatus() == Status.KEEP_GUESSING) {
		  Guess guess = guessingStrategy.nextGuess(this);
	  ...
	  // Hanging with Friends makes it so that longer words have fewer
	  // guess. The words to be guessed are between 4 and 8 letters.
	  int numberOfGuesses = 12 - this.getSecretWordLength();
	  return currentScore() - numberOfGuesses;	  
  }
   ...
   * @return the score for the game until it is won or lost
   */
  public int score(GuessingStrategy guessingStrategy) {
	  char rightMostVowel = StringOps.rightMostVowel(secretWord);
	  Guess firstGuess = new GuessLetter(rightMostVowel);
	  firstGuess.makeGuess(this);
